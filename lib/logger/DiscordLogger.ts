...
export class DiscordLogger {
   ...
   */
  private convertDJSType({ type }: GuildChannel): TResolvedChannelTypeUnion {
  ...
  public async handleChannelUpdate(oldChannel: GuildChannel, newChannel: GuildChannel): Promise<void> {
    ...
    const disallowedChannelTypes = [ChannelType.PrivateThread, ChannelType.PublicThread, ChannelType.AnnouncementThread, ChannelType.DM];
    
    if( // Lets avoid spamming the API
      disallowedChannelTypes.includes(oldChannel.type) || 
    ...
    try {
      ...
      if(oldChannel instanceof TextChannel && (newChannel instanceof TextChannel || newChannel instanceof NewsChannel)) {
        ...
        if(oldChannel instanceof TextChannel && newChannel instanceof NewsChannel) {
          diff[diff.length] = `Changed channel type to an announcement channel`
        }
      ...
      if(diff.length) { // Prevent spamming permissions until its figured out
        const embed = new LogEmbed(DiscordLoggerLevel.MODERATION_INFO)
          .setAuthor({ name: authorStr, iconURL: this.getAvatarFromAuditLog(auditLogEntry) })
  ...
  public async handleGuildMemberUpdate(oldMember: GuildMember, newMember: GuildMember): Promise<void> {
    try {
      ...
      const changesZero = auditLogEntry.changes[0];
      
      const isTimeOutAction = changesZero.key === 'communication_disabled_until'
      
      if(isTimeOutAction) {
        const timedOut = !changesZero.old && changesZero.new
